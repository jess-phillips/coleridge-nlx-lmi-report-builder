...
function pivotData(data: MonthlyDataPoint[]) {
  const skills = [...new Set(data.map((d) => d.group))].sort()
  const byDate = new Map<string, Record<string, number | string>>()
  for (const pt of data) {
    if (!byDate.has(pt.date)) byDate.set(pt.date, { date: pt.date })
    byDate.get(pt.date)![pt.group] = pt.n_postings
  }
...
export function SkillsLineChart({ data, loading }: Props) {
  if (loading) {
    return (
  ...
  const { rows, skills } = pivotData(data)

  const allValues = rows.flatMap((r) =>
    skills.map((s) => (typeof r[s] === 'number' ? (r[s] as number) : 0))
  ).filter((v) => v > 0)
